...
* **`rank.py`** — Main Python 3.12 script containing candidate parsing, safety filter, scoring logic, tie-breaker, and NLG rationale generator.
* **`team_10.csv`** — The finalized ranked output of the top 100 candidates.
* **`submission_metadata.yaml`** — Team metadata, environment details, and methodology summary.
* **`requirements.txt`** — Package requirements (the ranking code uses python standard libraries only, ensuring zero import overhead!).
